use uplc_turbo::{arena::Arena, binder::{DeBruijn, Eval}, flat, term::Term};
...
fn max_var_index<'a>(term: &'a Term<'a, DeBruijn>) -> usize {
    match term {
        ...
        Term::Apply { function, argument } => {
            max_var_index(function).max(max_var_index(argument))
        }
        Term::Delay(body) => max_var_index(body),
        Term::Force(body) => max_var_index(body),
        Term::Constr { fields, .. } => {
            fields.iter().map(|f| max_var_index(f)).max().unwrap_or(0)
        }
...
fn main() {
    ...
    if max > 255 {
        eprintln!("WARNING: index {} exceeds u8 range — bytecode Var opcode will truncate!", max);
    }
}
