use uplc_turbo::{
    arena::Arena,
    binder::DeBruijn,
    bytecode::{compiler, read_u32, read_u16, Op},
    flat,
};
...
fn main() {
    ...
    while ip < bc.len() {
        ...
        *counts.entry(op).or_insert(0u32) += 1;
        ...
        match op {
            ...
            0x06 => {                                  // Constr: u8 tag + u8 nfields + offsets
                ip += 1; // tag
                let nfields = bc[ip] as usize; ip += 1;
                ip += nfields * 4;
            }
            0x0B => {                                  // ConstrBig: u64 tag + u8 nfields + offsets
                ip += 8;
                let nfields = bc[ip] as usize; ip += 1;
                ip += nfields * 4;
            }
            0x07 => {                                  // Case: u8 nbranches + offsets
                let nb = bc[ip] as usize; ip += 1;
                ip += nb * 4;
            }
            0x08 => ip += 2,                          // Const: u16
            ...
            _ => { eprintln!("Unknown opcode {:#04x} at {}", op, ip-1); break; }
        }
    }
    ...
    for (op, count) in &sorted {
        let name = op_names.iter().find(|(o, _)| o == *op).map(|(_, n)| *n).unwrap_or("???");
        println!("{:<20} {:>6} {:>7.1}%", name, count, **count as f64 / total as f64 * 100.0);
    }
